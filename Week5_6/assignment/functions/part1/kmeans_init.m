...
switch init
    case "sample"
        ...
        %}
        selectIndex = randsample(M, k); % using `randsample`
        Mu = X(:,selectIndex);
        % using `datasample` % Mu = datasample(X,k,2);
    case "range"
        %{
        random in a range: Select  K  data points uniformly at random 
        from the range of  X ; i.e. if we have a 2D dataset we select 
        points at random between  [min(X1),min(X2)]->[max(X1),max(X2)]
        %}
        minX = min(X,[],2); rangeX = range(X,2); % maxX = max(X,[],2); 
